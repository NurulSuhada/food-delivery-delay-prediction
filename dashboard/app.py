...
with col2:
    ...
    st.metric("Average Delivery Time", f"{avg_duration:.2f} min")
...
with col4:
    avg_wait = filtered_df["Rider wait time (minutes)"].mean()
    st.metric("Average Rider Wait Time", f"{avg_wait:.2f} min")

# ANALYTICAL OUTPUT
if late_rate > 10:
    st.warning("Analytical Output: Late delivery rate is high. Operations team should monitor restaurant preparation and rider waiting time.")
else:
    st.success("Analytical Output: Late delivery rate is within an acceptable range based on the selected filter.")
...
with chart_col1:
    st.markdown("**Visualization 1: Distribution of Delivery Duration**")
...
with chart_col2:
    st.markdown("**Visualization 2: Number of Orders by Restaurant**")
...
st.markdown("**Visualization 3: Average Delivery Duration by Restaurant**")
avg_delivery_by_restaurant = (
    filtered_df.groupby("Restaurant name")["KPT duration (minutes)"]
    .mean()
